...
class Ex19_9_ArticleServiceTest_인증 {
...
/*추가*/ sut.updateArticle(dto.id(), dto);
    ...
    @Test
    void ArticleCommentController() {
        // Given
        Long articleId = 1L;
/*추가*/ String userId = "bitstudy";

/*삭제*/ //willDoNothing().given(articleRepository).deleteById(articleId, userId);
/*변경*/ willDoNothing().given(articleRepository).deleteByIdAndUserAccount_UserId(articleId, userId);

// When
/* 삭제 */// sut.deleteArticle(1L);
/* 변경 */ sut.deleteArticle(1L, userId);
// Then
/* 삭제 *///then(articleRepository).should().deleteById(articleId, userId);
